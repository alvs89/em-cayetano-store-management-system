...
function getPublicCreateTableNames(sql) {
  const tableNames = new Set();
  const createTablePattern = /\bCREATE\s+TABLE\s+public\.([a-zA-Z_][a-zA-Z0-9_]*)\b/gi;
  let match;
  while ((match = createTablePattern.exec(String(sql || ''))) !== null) {
...
function getRestoreValidationError(sql) {
  ...
  const supportedTables = new Set(RESTORE_APP_TABLES.map(table => table.toLowerCase()));
  const unsupportedTables = getPublicCreateTableNames(sql).filter(table => !supportedTables.has(table));
  if (unsupportedTables.length > 0) {
    return `The backup contains unsupported application tables: ${unsupportedTables.join(', ')}. Please restore only backups generated by this system version.`;
  }
